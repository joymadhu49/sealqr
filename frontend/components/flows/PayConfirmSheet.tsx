      ...
      {!payload ? null : done ? (
        <div className="flex flex-col items-center py-6 text-center">
          <motion.div
            ...
            className="grid h-20 w-20 place-items-center rounded-full bg-seal-500/15"
          >
            <CheckCircle2 className="h-11 w-11 text-seal-400" />
          </motion.div>
          <h3 className="mt-4 text-xl font-bold">Sent confidentially</h3>
          <p className="mt-1 max-w-xs text-sm text-white/55">
            You paid {shortAddr(payload.a)}. The amount is encrypted — the public ledger only shows that a transfer happened.
          </p>
        ...
        <div className="space-y-5">
          <div className="card flex items-center justify-between p-4">
            <div>
              ...
              <p className="mt-1 font-mono text-sm text-white">{shortAddr(payload.a, 6)}</p>
            </div>
            {payload.m && <span className="pill max-w-[50%] truncate">{payload.m}</span>}
          </div>

          <AmountInput value={amount} onChange={setAmount} />

          <div className="flex items-center gap-2 rounded-2xl border border-seal-500/20 bg-seal-500/[0.06] px-4 py-3">
            <ShieldCheck className="h-4 w-4 shrink-0 text-seal-400" />
            <p className="text-xs text-white/65">This amount is encrypted before it ever leaves your device.</p>
          </div>
